...
def rotation(image, angleInDegrees):
    ...
    out_img = cv2.warpAffine(image, rot, (b_w, b_h), flags=cv2.INTER_LINEAR)
    return out_img
...
while numImage < allFrames:
    ...
    fileName = str(str(numImage) + 'C.png')
    cv2.imwrite(fileName, ChangeImage)
    numImage += 1

# Создаем видео
frameIns = 0
out = cv2.VideoWriter("video_new.mp4", cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), fps,
                      (480, 848))  # создаем видео, fps - расчитано Добавить разрешение видео
